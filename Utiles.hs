...
-- campos en orden: Index,Organization Id,Name,Website,Country,Description,Founded,Industry,Number of employees
type TuplaDeOrganizacion = (Int, String, String, String, String, String, Int, String, Int)

-- Dado un string (el contenido de un archivo csv), lo parsea y devuelve una lista con todas las tuplas de organizaciones que contenga.
...
leerTuplasDeOrganizaciones archivo = map parsearOrganizacion filasDeOrganizaciones
    where
        ...
        -- divido el string del contenido por linea, asi puedo trabajar con cada una individualmente
...
-- Dada una fila del CSV de organizaciones (en formato string), lo parsea y devuelve una tupla con los diferentes componentes
...
parsearOrganizacion string = (index, id, nombre, website, pais, desc, fundada, industria, nroEmpleados)
    where
        ...
        -- leo cada campo, usando la funcion read para pasar de String a numeros cuando hace falta
...
-- Dado un separador (un caracter) y un string, devuelve la lista de strings que resulta de dividir el string en partes teniendo en cuenta el separador.
-- Ejemplo:
